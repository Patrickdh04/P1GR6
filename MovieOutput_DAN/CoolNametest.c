...
void give_points_series(seriesData seriesArray[], pref userpref, int size)
{
    for (int i = 0; i < size; i++)
    {
        ...
        for (int l = 0; l < 3 && seriesArray[i].actors[l] != NULL; l++)
        {
            for (int j = 0; j < 3 && userpref.actors[j] != NULL; j++)
            {
                printf("Comparing actor: %s with user actor: %s\n", seriesArray[i].actors[l], userpref.actors[j]); // DEBUG
                if (strcmp(seriesArray[i].actors[l], userpref.actors[j]) == 0)
                {
                    printf("Match found! Incrementing score for actor: %s\n", seriesArray[i].actors[l]); // DEBUG
                    seriesArray[i].score++;
                    actorMatched[j] = 1;
                }
            }
...
// Sort series by score and rating
int sort_series(const void *a, const void *b)
{
...
// Display function for movies and series
void display(movieData movieArray[], int size_movie, seriesData seriesArray[], int size_series)
{
    // display movie
    ...
    {
        printf("%-16s%-16d%-16.1f%-16d\n", movieArray[i].title, movieArray[i].duration, movieArray[i].rating, movieArray[i].score);
    }
    printf("\n");

    // display series
    printf("These are the series recommendations: \n\n");
    printf("%-16s%-16s%-16s%-16s%-16s\n", "Title:", "Episodes:", "IMDB:", "TEST-SCORE: ", "Duration:");
    printf("-------------------------------------------------------------\n");
    for (int i = 0; i < size_series; i++)
    {
        printf("%-16s%-16d%-16.1f%-16d%-16d\n", seriesArray[i].title, seriesArray[i].episodes, seriesArray[i].rating, seriesArray[i].score, seriesArray[i].duration);
    }
    printf("\n");
}